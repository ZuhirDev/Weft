...
const AccountAction = () => {
  ...
  return (
    <>
    {selectedAccount ? (
    <div className="relative group px-4 lg:px-0 max-w-4xl mx-auto">
      ...
      <div className="relative z-10 overflow-hidden bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 text-sm shadow-sm py-6 rounded-2xl px-6">
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <Button
            variant="outline"
            className="h-20 w-full flex flex-col items-center justify-center gap-1 rounded-xl border-yellow-400 text-yellow-600 hover:bg-yellow-50 transition"
            onClick={open}
          >
            <PlusCircle className="h-6 w-6" />
            <span className="text-sm font-medium text-center">Account Actions</span>
          </Button>
        </div>

        <Dialog open={isOpen} onOpenChange={close}>
          <DialogContent className="sm:max-w-3xl max-h-[800px] overflow-y-auto rounded-2xl bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-8 py-6 shadow-lg">
            <DialogHeader>
            ...
            >
              <TabsList className="grid grid-cols-3 gap-2 mb-6 rounded-lg bg-muted p-1 w-full max-w-md">
                <TabsTrigger value="update" className="rounded-lg text-sm font-medium text-center">
                  Update
                </TabsTrigger>
                <TabsTrigger value="create" className="rounded-lg text-sm font-medium text-center">
                  Create
                </TabsTrigger>
                <TabsTrigger value="delete" className="rounded-lg text-sm font-medium text-center">
                  Delete
                </TabsTrigger>
              </TabsList>
